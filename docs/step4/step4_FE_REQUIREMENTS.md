...
- [ ] 선택한 상품 목록 표시 (상품명, 가격, 수량)
- [ ] 쿠폰 적용 버튼
- [ ] 배송지 정보 체크박스 (제주도 및 도서 산간 지역 여부)
- [ ] 주문 금액 / 쿠폰 할인 금액 / 배송비 / 총 결제 금액 표시
...
- [ ] 쿠폰 목록 표시 (쿠폰명, 만료일, 최소 주문 금액 등 부가 정보)
- [ ] 사용 불가 쿠폰 비활성화 처리 (선택 불가, 사유 표시)
...
- [ ] `/checkout/:orderId` 라우트 추가 (현재 `/checkout` 고정)
- [ ] 결제 완료 페이지 라우트 추가
